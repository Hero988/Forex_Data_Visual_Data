...
# Change timeframe to daily candles instead of 1-hour candles
timeframe = mt5.TIMEFRAME_D1  # Daily timeframe for each candlestick

# Change the interval to a daily basis (12 hours is now 1 day)
interval = timedelta(days=12)  # 1 day of data per image

# Update the `hours` variable to match the daily interval
interval_int = int(interval.total_seconds() / (24 * 3600))
#print(interval_int)
#time.sleep(6000)

#timeframe = mt5.TIMEFRAME_H1  # 1-hour timeframe for each candlestick
#interval = timedelta(hours=12)  # 12 hours of data per image
...
def save_combined_chart(df, future_df, filename):
    ...
    This version is tailored for daily candlestick data.
    """
    ...
    # Plot setup
    fig, ax = plt.subplots(figsize=(12, 8))  # Increased size for better daily data visualization
    ...
    for i in range(len(combined_df)):
        ...
        # Draw the body
        width = 0.6  # Adjust width for daily candlestick visualization
    ...
    ax.set_xlim(mdates.date2num(combined_df.index.min()) - 0.5, mdates.date2num(combined_df.index.max()) + 0.5)
    
    # Remove the y-axis and spines, but keep the x-axis for daily readability
    ...
    plt.savefig(filename, dpi=150, bbox_inches='tight', pad_inches=0.1)  # Higher DPI for better detail
    plt.close()
...
for symbol in symbols:
    ...
    while current_date <= end_date:
        ...
        if df is not None and len(df) >= 1:
            ...
            future_date = next_date + timedelta(days=prediction_window)
